...
export default async function DonateSuccessPage({
  ...
  const donateDict = (await getDictionary(locale, "donate-success")) as
    | DonateSuccessDict
    | undefined;

  // 🧠 Text from donate-success (with English fallbacks)
  const title =
    donateDict?.title ?? "Thank you for your donation!";
  const body =
    donateDict?.body ??
  ...
  const receiptNote =
    donateDict?.receiptNote ??
    "A payment confirmation and receipt have been sent to your email by our secure payment provider.";

  // 🔄 Buttons from hero dictionary so they stay consistent site-wide
  const backToHomepageLabel =
    homeDict.hero?.backToHomepage ?? "Back to homepage";
  const seeHowFundsAreUsedLabel =
    homeDict.hero?.seeHowFundsAreUsed ?? "See how funds are used";

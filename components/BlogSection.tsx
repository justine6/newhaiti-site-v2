...
export default async function BlogSection({ locale, dictionary }: BlogSectionProps) {
  const safeLocale = locale && locale.trim() !== "" ? locale : "en";

  // Super defensive access so we never crash if blogSection is missing
      ...
      <section className="py-12 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl font-bold text-red-600 mb-4">
            ⚠️ {blogDict.title || "Blog Unavailable"}
          </h2>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
              {blogDict.title || "Latest updates from the movement"}
            </h2>
            <p className="text-gray-600 dark:text-gray-300">
              {blogDict.paragraph ||
                "Stories, reflections, and progress updates from the Nouvo Ayiti 2075 journey."}
            </p>
          </div>
          <Link
        ...
        {isFallback && (
          <p className="text-center text-sm text-yellow-600 bg-yellow-50 border border-yellow-200 px-4 py-2 rounded-md mb-6">
            {blogDict.fallbackNotice ||
              "We’re still translating posts for your language. Showing English posts for now."}
          </p>
        )}
        ...
        {latestPosts.length === 0 ? (
          <p className="text-center text-gray-500 italic">
            {blogDict.blogUnavailable || "No posts available yet. Check back soon."}
          </p>
        ) : (
            ...
            {latestPosts.map((post) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                  {post.excerpt ||
                    blogDict.blogUnavailable ||
                    "This story is still being summarized."}
                </p>
                <Link
